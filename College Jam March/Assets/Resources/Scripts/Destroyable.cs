...
//Courtesy of Ditzel https://gist.github.com/ditzel/73f4d1c9028cc3477bb921974f84ed56
...
public class Destroyable : MonoBehaviour
{
    ...
    public void DestroyMesh()
    {
        ...
        for (var c = 0; c < CutCascades; c++)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                var randomBounds = new Vector3(0, 0, 0);
                if (!restrictedAxis.x)
                { randomBounds.x = UnityEngine.Random.Range(bounds.min.x, bounds.max.x); }
                if (!restrictedAxis.y)
                { randomBounds.y = UnityEngine.Random.Range(bounds.min.y, bounds.max.y); }
                if (!restrictedAxis.z)
                { randomBounds.z = UnityEngine.Random.Range(bounds.min.z, bounds.max.z); }

                var plane = new Plane(UnityEngine.Random.onUnitSphere, new Vector3(randomBounds.x, randomBounds.y, randomBounds.z));
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(Destroyable original)
        {
            ...
            var meshDestroy = GameObject.AddComponent<Destroyable>();
